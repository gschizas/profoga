...
namespace org.jmol.g3d
{
	...
	class Sphere3D
	{
		...
		internal Sphere3D(Graphics3D g3d)
		{
			this.g3d = g3d;
		}
		
		internal virtual void  render(int[] shades, bool tScreened, int diameter, int x, int y, int z)
		{
			...
			if (maxX < 0 || minX >= g3d.width || maxY < 0 || minY >= g3d.height || maxZ < g3d.slab || minZ > g3d.depth)
				return ;
			...
			if (minX < 0 || maxX >= g3d.width || minY < 0 || maxY >= g3d.height || minZ < g3d.slab || z > g3d.depth)
				renderShapeClipped(shades, tScreened, ss, diameter, x, y, z);
			else
				renderShapeUnclipped(shades, tScreened, ss, diameter, x, y, z);
		}
		
		internal virtual void  renderShapeUnclipped(int[] shades, bool tScreened, int[] sphereShape, int diameter, int x, int y, int z)
		{
			int[] pbuf = g3d.pbuf;
			int[] zbuf = g3d.zbuf;
			int offsetSphere = 0;
		...
		//UPGRADE_NOTE: Final was removed from the declaration of 'SHADE_SLAB_CLIPPED '. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1003'"
		//UPGRADE_NOTE: The initialization of  'SHADE_SLAB_CLIPPED' was moved to static method 'org.jmol.g3d.Sphere3D'. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1005'"
		private static readonly int SHADE_SLAB_CLIPPED;
		
		internal virtual void  renderShapeClipped(int[] shades, bool tScreened, int[] sphereShape, int diameter, int x, int y, int z)
		{
			int[] pbuf = g3d.pbuf;
			int[] zbuf = g3d.zbuf;
			int offsetSphere = 0;
			...
			do 
			{
				bool tSouthVisible = ySouth >= 0 && ySouth < height;
				bool tNorthVisible = yNorth >= 0 && yNorth < height;
				int offsetSE = offsetSouthCenter;
				int offsetSW = offsetSouthCenter - evenSizeCorrection;
				int offsetNE = offsetNorthCenter;
				int offsetNW = offsetNorthCenter - evenSizeCorrection;
				...
				do 
				{
					bool tWestVisible = xWest >= 0 && xWest < width;
					bool tEastVisible = xEast >= 0 && xEast < width;
					packed = sphereShape[offsetSphere++];
					int zPixel;
					bool isSlabClipped;
					if (z < slab)
		...
		internal const int maxSphereCache = 128;
		internal const int maxOddSizeSphere = 49;
		internal static int[][] sphereShapeCache = new int[maxSphereCache][];
		
		internal virtual int[] getSphereShape(int diameter)
		{
			int[] ss;
			if (diameter > maxSphereCache)
		...
		internal static void  flushImageCache()
		{
			sphereShapeCache = new int[maxSphereCache][];
		}
		
		internal virtual int[] createSphereShape(int diameter)
		{
			int countSE = 0;
			bool oddDiameter = (diameter & 1) != 0;
			...
			for (int i = 0; i < radius; ++i, ++y)
			{
				...
				for (int j = 0; j < radius; ++j, ++x)
				{
					...
					if (z2 >= 0)
					{
						...
						float z = (float) System.Math.Sqrt(z2);
						//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
						int height = (int) z;
				...
				sphereShape[offset - 1] |= unchecked((int) 0x80000000);
			}
			return sphereShape;
		}
		...
		internal static bool sphereShadingCalculated = false;
		//UPGRADE_NOTE: Final was removed from the declaration of 'sphereIntensities '. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1003'"
		internal static readonly sbyte[] sphereIntensities = new sbyte[256 * 256];
		
		internal virtual void  calcSphereShading()
		{
		...
		internal static sbyte calcSphereIntensity(int x, int y, int r)
		{
			int d = 2 * r + 1;
			x += r;
			if (x < 0)
		...
		internal virtual void  renderLargeSphere(int[] shades, bool tScreened, int diameter, int x, int y, int z)
		{
			if (!sphereShadingCalculated)
				calcSphereShading();
		...
		internal virtual void  renderQuadrant(int[] shades, bool tScreened, int radius, int x, int y, int z, int xSign, int ySign)
		{
			...
			if (xStatus < 0 && x2Status < 0 || xStatus > 0 && x2Status > 0 || yStatus < 0 && y2Status < 0 || yStatus > 0 && y2Status > 0 || zStatus < 0 || z2Status < 0)
				return ;
			if (xStatus == 0 && x2Status == 0 && yStatus == 0 && y2Status == 0 && zStatus == 0 && z2Status == 0)
		...
		internal virtual void  renderQuadrantUnclipped(int[] shades, bool tScreened, int radius, int x, int y, int z, int xSign, int ySign)
		{
			...
			bool flipflopBeginLine = ((x ^ y) & 1) == 0;
			...
			for (int i = 0, i2 = 0; i2 <= r2; i2 += i + i + 1, ++i)
			{
				...
				bool flipflop = (flipflopBeginLine = !flipflopBeginLine);
				int s2 = r2 - i2;
				int z0 = z - radius;
				int y8 = ((i * ySign + radius) << 8) / dDivisor;
				for (int j = 0, j2 = 0; j2 <= s2; j2 += j + j + 1, ++j)
				{
					offsetPbuf += xSign;
					if (!tScreened || (flipflop = !flipflop))
					{
						if (zbuf[offsetPbuf] <= z0)
							continue;
						//UPGRADE_WARNING: Data types in Visual C# might be different.  Verify the accuracy of narrowing conversions. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1042'"
						int k = (int) System.Math.Sqrt(s2 - j2);
						z0 = z - k;
						if (zbuf[offsetPbuf] <= z0)
							continue;
		...
		internal virtual void  renderQuadrantClipped(int[] shades, bool tScreened, int radius, int x, int y, int z, int xSign, int ySign)
		{
			int r2 = radius * radius;
			int dDivisor = radius * 2 + 1;
			...
			for (int i = 0, i2 = 0; i2 <= r2; i2 += i + i + 1, ++i, offsetPbufBeginLine += lineIncrement)
			{
				yCurrent += ySign;
				if (yCurrent < 0)
				{
				...
				int y8 = ((i * ySign + radius) << 8) / dDivisor;
				randu = ((randu << 16) + (randu << 1) + randu) & 0x7FFFFFFF;
				for (int j = 0, j2 = 0; j2 <= s2; j2 += j + j + 1, ++j, offsetPbuf += xSign)
				{
					xCurrent += xSign;
					if (xCurrent < 0)
					{
