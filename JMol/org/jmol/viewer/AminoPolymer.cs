...
namespace org.jmol.viewer
{
	...
	class AminoPolymer:AlphaPolymer
	{
		...
		internal short[] mainchainHbondOffsets;
		internal short[] min1Indexes;
		internal short[] min1Energies;
		internal short[] min2Indexes;
		internal short[] min2Energies;
		
		internal AminoPolymer(Monomer[] monomers):base(monomers)
		{
		}
		
		internal override bool hasWingPoints()
		{
			return true;
		}
		
		internal const bool debugHbonds = false;
		
		internal override void  calcHydrogenBonds(System.Collections.BitArray bsA, System.Collections.BitArray bsB)
		{
		...
		internal virtual void  initializeHbondDataStructures()
		{
		...
		internal virtual void  freeHbondDataStructures()
		{
		...
		internal Vector3f vectorPreviousOC = new Vector3f();
		...
		internal Point3f aminoHydrogenPoint = new Point3f();
		
		internal virtual void  calcProteinMainchainHydrogenBonds(System.Collections.BitArray bsA, System.Collections.BitArray bsB)
		{
			Point3f carbonPoint;
			Point3f oxygenPoint;
		...
		internal virtual void  bondAminoHydrogen(int indexDonor, Point3f hydrogenPoint, System.Collections.BitArray bsA, System.Collections.BitArray bsB)
		{
		...
		internal virtual int calcHbondEnergy(Point3f nitrogenPoint, Point3f hydrogenPoint, AminoMonomer target)
		{
		...
		internal virtual void  createResidueHydrogenBond(int indexAminoGroup, int indexCarbonylGroup, System.Collections.BitArray bsA, System.Collections.BitArray bsB)
		{
			short order;
			int aminoBackboneHbondOffset = indexAminoGroup - indexCarbonylGroup;
			if (debugHbonds)
				System.Console.Out.WriteLine("aminoBackboneHbondOffset=" + aminoBackboneHbondOffset + " amino:" + monomers[indexAminoGroup].getSeqcodeString() + " carbonyl:" + monomers[indexCarbonylGroup].getSeqcodeString());
			...
			if (debugHbonds)
				System.Console.Out.WriteLine("createResidueHydrogenBond(" + indexAminoGroup + "," + indexCarbonylGroup);
		...
		*/
		...
		internal override void  calculateStructures()
		{
		...
		internal virtual void  calcPhiPsiAngles(AminoMonomer leadingResidue, AminoMonomer trailingResidue, float[] phi_psi)
		{
		...
		internal virtual bool isHelix(float[] phi_psi)
		{
			float phi = phi_psi[0];
			float psi = phi_psi[1];
		...
		internal virtual bool isSheet(float[] phi_psi)
		{
			float phi = phi_psi[0];
			float psi = phi_psi[1];
		...
		internal virtual bool isTurn(float[] phi_psi)
		{
			float phi = phi_psi[0];
			float psi = phi_psi[1];
		...
		*/
		...
		internal virtual bool isHbonded(int indexDonor, int indexAcceptor)
		{
