...
namespace org.jmol.viewer
{
	...
	{
		virtual internal int[] LeadAtomIndices
		{
			get
			{
				if (atomIndices == null)
		...
		virtual internal Point3f InitiatorPoint
		{
			get
			{
		...
		virtual internal Point3f TerminatorPoint
		{
			get
			{
		...
		virtual internal Point3f[] LeadMidpoints
		{
			get
			{
		...
		virtual internal Vector3f[] WingVectors
		{
			get
			{
				if (leadMidpoints == null)
		...
		internal Polymer(Monomer[] monomers)
		{
		...
		internal static Polymer allocatePolymer(Group[] groups, int firstGroupIndex)
		{
		...
		internal static Monomer[] getAlphaMonomers(Group[] groups, int firstGroupIndex)
		{
		...
		internal static Monomer[] getAminoMonomers(Group[] groups, int firstGroupIndex)
		{
		...
		internal static Monomer[] getPhosphorusMonomers(Group[] groups, int firstGroupIndex)
		{
		...
		internal static Monomer[] getNucleicMonomers(Group[] groups, int firstGroupIndex)
		{
		...
		internal virtual int getIndex(Monomer monomer)
		{
			int i;
		...
		internal virtual int getIndex(char chainID, int seqcode)
		{
			int i;
			for (i = monomerCount; --i >= 0; )
		...
		internal Point3f getLeadPoint(int monomerIndex)
		{
			return monomers[monomerIndex].LeadAtomPoint;
		}
		
		internal Atom getLeadAtom(int monomerIndex)
		{
			return monomers[monomerIndex].LeadAtom;
		}
		
		internal virtual void  getLeadMidPoint(int groupIndex, Point3f midPoint)
		{
		...
		internal virtual bool hasWingPoints()
		{
			return false;
		}
		...
		internal Point3f getWingPoint(int polymerIndex)
		{
			return monomers[polymerIndex].WingAtomPoint;
		}
		
		internal virtual void  addSecondaryStructure(sbyte type, char startChainID, int startSeqcode, char endChainID, int endSeqcode)
		{
		}
		
		internal virtual void  calculateStructures()
		{
		}
		
		internal virtual void  calcHydrogenBonds(System.Collections.BitArray bsA, System.Collections.BitArray bsB)
		{
			// subclasses should override if they know how to calculate hbonds
		}
		
		internal void  calcLeadMidpointsAndWingVectors()
		{
			...
			bool hasWingPoints = hasWingPoints();
		...
		internal int selectedMonomerCount;
		...
		internal System.Collections.BitArray bsSelectedMonomers;
		...
		internal virtual int getSelectedMonomerIndex(Monomer monomer)
		{
