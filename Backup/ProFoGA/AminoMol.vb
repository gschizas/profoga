...
Public Class AminoMol
	...
	Public Class Amino
		...
		Public Sub DehydrateC()
			Me.Atom(DehydC1).Name = "*"
			Me.Atom(DehydC2).Name = "*"

			For i As Byte = 1 To 6
				Me.Atom(DehydC1).link(i).atom = -1
			Next
		...
		Public Sub DehydrateN()
			Me.Atom(Me.DehydN - 1).Name = "* "
		End Sub
		Public Sub Display(ByVal g As System.Drawing.Graphics)
			Const siz = 15
			...
			sx = g.ClipBounds.Width
			sy = g.ClipBounds.Height

			cx = g.ClipBounds.Width \ 2		' sx \ 2 + g.ClipBounds.Left
			cy = g.ClipBounds.Height \ 2		'sy \ 2 + g.ClipBounds.Top
			...
			g.DrawRectangle(gPen, rect1.X, rect1.Y, rect1.Width, rect1.Height)

			Exit Sub

			For z = 100 To -100 Step -1
				For i = 0 To Me.Atom.Length - 1
					...
					If Me.DisplayOptions.Numbers And Atom(i).Name <> "*" Then
						...
						sizF = g.MeasureString(c, gFont)
						tx = sizF.Width
						ty = sizF.Height
						pnt1 = New PointF
						pnt1.X = cx - tx \ 2 + (sz * scx * (Atom(i).X))
						pnt1.Y = cy - ty \ 2 + (sz * scy * (Atom(i).Y))
						g.DrawString(c, gFont, gBrush, pnt1)
					End If
					'End If
		...
		Public Sub Move(ByVal dx As Double, ByVal dy As Double, ByVal dz As Double)
			For i As Byte = 0 To Me.Atom.Length - 1
				Atom(i).X += dx
				Atom(i).Y += dy
				Atom(i).Z += dz
			Next
		...
		Public Sub Open(ByVal fname As String)
			...
			n = amn.ReadLine()
			Me.Atom = Array.CreateInstance(GetType(Atom), n)
			...
			For i = 1 To n
				c = amn.ReadLine - 1
				Me.Atom(Math.Abs(c)).Charge = Math.Sign(c)
			Next
		...
		Public Sub Join(ByRef OneAmino As Amino, ByRef TwoAmino As Amino)
			...
			bond1 = OneAmino.Atom(OneAmino.DehydC2).link(0).atom
			bond2 = TwoAmino.Atom(TwoAmino.DehydN).link(0).atom

			'	Move to lock 1st atom
			TwoAmino.Move( _
			 OneAmino.Atom(bond1).X - TwoAmino.Atom(TwoAmino.DehydN).X, _
			 OneAmino.Atom(bond1).Y - TwoAmino.Atom(TwoAmino.DehydN).Y, _
			 OneAmino.Atom(bond1).Z - TwoAmino.Atom(TwoAmino.DehydN).Z)


			'	Rotate to lock 2nd atom
			x1 = OneAmino.Atom(OneAmino.DehydC2).X
			y1 = OneAmino.Atom(OneAmino.DehydC2).Y
			z1 = OneAmino.Atom(OneAmino.DehydC2).Z

			x2 = OneAmino.Atom(bond1).X
			y2 = OneAmino.Atom(bond1).Y
			z2 = OneAmino.Atom(bond1).Z

			x3 = TwoAmino.Atom(bond2).X
			y3 = TwoAmino.Atom(bond2).Y
			z3 = TwoAmino.Atom(bond2).Z
			...
			For i = 0 To 5
				If OneAmino.Atom(OneAmino.Atom(bond1).link(i).atom).Name = "O" Then bond3 = OneAmino.Atom(bond1).link(i).atom
				If TwoAmino.Atom(TwoAmino.Atom(bond2).link(i).atom).Name = "H" Then bond4 = TwoAmino.Atom(bond2).link(i).atom
			Next
			...
			x1 = OneAmino.Atom(bond1).X
			y1 = OneAmino.Atom(bond1).Y
			z1 = OneAmino.Atom(bond1).Z

			x2 = TwoAmino.Atom(bond2).X
			y2 = TwoAmino.Atom(bond2).Y
			z2 = TwoAmino.Atom(bond2).Z

			x3 = OneAmino.Atom(bond3).X
			y3 = OneAmino.Atom(bond3).Y
			z3 = OneAmino.Atom(bond3).Z

			x4 = TwoAmino.Atom(bond4).X
			y4 = TwoAmino.Atom(bond4).Y
			z4 = TwoAmino.Atom(bond4).Z
	...
	Public Structure Atom
		...
		<Xml.Serialization.XmlAttribute(AttributeName:="charge")> _
		Public Charge As Short
		<Xml.Serialization.XmlAttribute(AttributeName:="name")> _
		Public Name As String
		<Xml.Serialization.XmlAttribute(AttributeName:="x")> _
		Public X As Double
		<Xml.Serialization.XmlAttribute(AttributeName:="y")> _
		Public Y As Double
		<Xml.Serialization.XmlAttribute(AttributeName:="z")> _
		Public Z As Double
	End Structure
	...
	<Serializable(), Xml.Serialization.XmlRoot("link")> _
	 Public Structure BondLink
		...
		<Xml.Serialization.XmlAttribute("bond")> _
