...
namespace org.jmol.viewer
{
	...
	{
		virtual internal int MonomerCount
		{
			get
			{
				return monomerCount;
		...
		virtual internal int MonomerIndex
		{
			get
			{
				return monomerIndex;
		...
		virtual internal Point3f[] Segments
		{
			get
			{
				if (segments == null)
		...
		virtual internal Point3f AxisStartPoint
		{
			get
			{
				calcAxis();
		...
		virtual internal Point3f AxisEndPoint
		{
			get
			{
				calcAxis();
		...
		internal AlphaPolymer apolymer;
		internal sbyte type;
		internal int monomerIndex;
		internal int monomerCount;
		internal Point3f center;
		internal Point3f axisA, axisB;
		internal Vector3f axisUnitVector;
		internal Point3f[] segments;
		
		internal ProteinStructure(AlphaPolymer apolymer, sbyte type, int monomerIndex, int monomerCount)
		{
		...
		internal virtual void  calcAxis()
		{
		}
		
		internal virtual void  calcSegments()
		{
			if (segments != null)
				return ;
			calcAxis();
		...
		internal virtual bool lowerNeighborIsHelixOrSheet()
		{
			if (monomerIndex == 0)
				return false;
		...
		internal virtual bool upperNeighborIsHelixOrSheet()
		{
		...
		internal Vector3f vectorProjection = new Vector3f();
		
		internal virtual void  projectOntoAxis(Point3f point)
		{
		...
		internal virtual Point3f getStructureMidPoint(int index)
		{
			if (segments == null)
				calcSegments();
			/*
