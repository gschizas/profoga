...
namespace org.jmol.viewer
{
	...
	class MeasuresRenderer:ShapeRenderer
	{
		...
		internal bool showMeasurementNumbers;
		internal short measurementMad;
		...
		internal virtual void  renderMeasurement(Measurement measurement, short colix)
		{
			renderMeasurement(measurement.count, measurement, colix, true);
		}
		
		internal virtual void  renderMeasurement(int count, Measurement measurement, short colix, bool renderArcs)
		{
		...
		internal virtual int drawSegment(int x1, int y1, int z1, int x2, int y2, int z2, short colix)
		{
			if (measurementMad < 0)
			{
		...
		internal virtual void  renderDistance(short colix)
		{
			renderDistance(frame.getAtomAt(measurement.countPlusIndices[1]), frame.getAtomAt(measurement.countPlusIndices[2]), colix);
		}
		
		internal virtual void  renderDistance(Atom atomA, Atom atomB, short colix)
		{
			/*
		...
		internal virtual void  renderAngle(short colix, bool renderArcs)
		{
		...
		internal virtual void  renderAngle(Atom atomA, Atom atomB, Atom atomC, short colix, bool renderArcs)
		{
			/*
		...
		internal virtual void  renderTorsion(short colix, bool renderArcs)
		{
		...
		internal virtual void  renderTorsion(Atom atomA, Atom atomB, Atom atomC, Atom atomD, short colix, bool renderArcs)
		{
			/*
			if (! (atomA.isVisible() && atomB.isVisible() &&
		...
		internal virtual void  paintMeasurementString(int x, int y, int z, int radius, short colix)
		{
		...
		internal virtual void  renderPendingMeasurement(PendingMeasurement pendingMeasurement)
		{
		...
		internal virtual void  renderPendingWithCursor(PendingMeasurement pendingMeasurement, short colixRubberband)
		{
			int count = pendingMeasurement.count;
			if (count < 2)
				return ;
