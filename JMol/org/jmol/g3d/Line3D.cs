...
namespace org.jmol.g3d
{
	...
	sealed class Line3D
	{
		...
		internal Line3D(Graphics3D g3d)
		{
			this.g3d = g3d;
		}
		
		internal void  drawLine(int argbA, bool tScreenedA, int argbB, bool tScreenedB, int xA, int yA, int zA, int xB, int yB, int zB)
		{
			...
			switch (visibilityCheck(xA, yA, zA, xB, yB, zB))
			{
				...
				case VISIBILITY_UNCLIPPED: 
					plotLineDeltaUnclipped(argbA, tScreenedA, argbB, tScreenedB, xA, yA, zA, dxBA, dyBA, dzBA);
					break;
				
				case VISIBILITY_CLIPPED: 
					plotLineDeltaClipped(argbA, tScreenedA, argbB, tScreenedB, xA, yA, zA, dxBA, dyBA, dzBA);
					break;
				}
		}
		
		internal void  drawVLine(int argb, bool tScreened, int x, int y, int z, int h, bool checkSlab)
		{
			int width = g3d.width;
		...
		internal void  drawHLine(int argb, bool tScreened, int x, int y, int z, int w, bool checkSlab)
		{
			int width = g3d.width;
			int height = g3d.height;
			if ((y < 0) || (y >= height))
		...
		internal void  drawDashedLine(int argbA, bool tScreenedA, int argbB, bool tScreenedB, int run, int rise, int xA, int yA, int zA, int xB, int yB, int zB)
		{
			int dxBA = xB - xA, dyBA = yB - yA, dzBA = zB - zA;
			switch (visibilityCheck(xA, yA, zA, xB, yB, zB))
			{
				...
				case VISIBILITY_UNCLIPPED: 
					plotDashedLineDeltaUnclipped(argbA, tScreenedA, argbB, tScreenedB, run, rise, xA, yA, zA, dxBA, dyBA, dzBA);
					break;
				
				case VISIBILITY_CLIPPED: 
					plotDashedLineDeltaClipped(argbA, tScreenedA, argbB, tScreenedB, run, rise, xA, yA, zA, dxBA, dyBA, dzBA);
					break;
				}
		}
		
		internal const int VISIBILITY_UNCLIPPED = 0;
		internal const int VISIBILITY_CLIPPED = 1;
		internal const int VISIBILITY_OFFSCREEN = 2;
		...
		/// </returns>
		internal int visibilityCheck(int x1, int y1, int z1, int x2, int y2, int z2)
		{
		...
		internal const int zLT = 32;
		internal const int zGT = 16;
		internal const int xLT = 8;
		internal const int xGT = 4;
		internal const int yLT = 2;
		internal const int yGT = 1;
		
		internal int clipCode(int x, int y, int z)
		{
			int code = 0;
			if (x < 0)
				code |= xLT;
			...
			// note that this is .GT., not .GE.
				code |= zGT;
			
			return code;
		}
		
		internal void  plotDashedLineDeltaUnclipped(int argb1, bool tScreened1, int argb2, bool tScreened2, int run, int rise, int x, int y, int z, int dx, int dy, int dz)
		{
			if (rise >= run)
			{
				plotLineDeltaUnclipped(argb1, tScreened1, argb2, tScreened2, x, y, z, dx, dy, dz);
				return ;
			}
			int runIndex = 0;
			int[] pbuf = g3d.pbuf;
		...
		internal void  plotDashedLineDeltaClipped(int argb1, bool tScreened1, int argb2, bool tScreened2, int run, int rise, int x, int y, int z, int dx, int dy, int dz)
		{
			if (rise >= run)
			{
				plotLineDeltaClipped(argb1, tScreened1, argb2, tScreened2, x, y, z, dx, dy, dz);
				return ;
			}
			int runIndex = 0;
			int[] pbuf = g3d.pbuf;
			...
			if (dy <= dx)
			{
				...
				for (int n = dx - 1, nMid = n / 2; --n >= 0; )
				{
					...
					if (runIndex < rise && xCurrent >= 0 && xCurrent < width && yCurrent >= 0 && yCurrent < height)
					{
						int zCurrent = zCurrentScaled >> 10;
						if (zCurrent >= slab && zCurrent <= depth && zCurrent < zbuf[offset])
						{
							zbuf[offset] = zCurrent;
							pbuf[offset] = n > nMid?argb1:argb2;
						}
		...
		*/
		...
		internal void  plotLineDeltaUnclipped(int[] shades1, bool tScreened1, int[] shades2, bool tScreened2, int fp8Intensity, int x1, int y1, int z1, int dx, int dy, int dz)
		{
			int[] pbuf = g3d.pbuf;
			int[] zbuf = g3d.zbuf;
			int width = g3d.width;
			...
			int intensityUp = (intensity < Shade3D.shadeLast?intensity + 1:intensity);
			...
			bool tScreened = tScreened1;
			bool flipflop = ((x1 ^ y1) & 1) != 0;
		...
		internal void  plotLineDeltaUnclipped(int argb1, bool tScreened1, int argb2, bool tScreened2, int x1, int y1, int z1, int dx, int dy, int dz)
		{
			int[] pbuf = g3d.pbuf;
			int[] zbuf = g3d.zbuf;
			int width = g3d.width;
		...
		internal void  plotLineDeltaClipped(int argb1, bool tScreened1, int argb2, bool tScreened2, int x1, int y1, int z1, int dx, int dy, int dz)
		{
			int[] pbuf = g3d.pbuf;
			int[] zbuf = g3d.zbuf;
			int width = g3d.width, height = g3d.height;
			...
			{
				if (x1 >= 0 && x1 < width && y1 >= 0 && y1 < height && z1 >= slab && z1 <= depth && z1 < zbuf[offset])
				{
					zbuf[offset] = z1;
					pbuf[offset] = argb1;
				}
		...
		internal void  plotLineDeltaClipped(int[] shades1, bool tScreened1, int[] shades2, bool tScreened2, int fp8Intensity, int x1, int y1, int z1, int dx, int dy, int dz)
		{
			int intensity = fp8Intensity >> 8;
			int argb1 = shades1[intensity];
			int argb2 = shades2[intensity];
