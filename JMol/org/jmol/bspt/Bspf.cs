...
namespace org.jmol.bspt
{
	...
	/// <summary> A Binary Space Partitioning Forest
	/// <p>
	...
	public sealed class Bspf
	{
		...
		internal int dimMax;
		internal Bspt[] bspts;
		internal SphereIterator[] sphereIterators;
		...
		*/
		...
		public SphereIterator getSphereIterator(int bsptIndex)
		{
			if (bsptIndex >= sphereIterators.Length)
			{
				...
				Array.Copy(sphereIterators, 0, t, 0, sphereIterators.Length);
				sphereIterators = t;
			}
			if (sphereIterators[bsptIndex] == null && bspts[bsptIndex] != null)
