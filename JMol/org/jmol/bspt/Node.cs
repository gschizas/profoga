...
namespace org.jmol.bspt
{
	...
	/// <summary> Nodes of the bspt. It is a binary tree so nodes contain two children,
	...
	class Node:Element
	{
		...
		internal Node(Bspt bspt, int level, Leaf leafLeft)
		{
			this.bspt = bspt;
			if (level == bspt.treeDepth)
			{
				...
				if (bspt.treeDepth >= Bspt.MAX_TREE_DEPTH)
					System.Console.Out.WriteLine("BSPT tree depth too great:" + bspt.treeDepth);
			}
		...
		internal override Element addTuple(int level, Tuple tuple)
		{
